...
void setup() {
  ...
  canMsg.can_id = 0x000;
  canMsg.can_dlc = 8;  // Changing to 8 bytes since we are now sending two float values
  for(int i = 0; i < 8; i++){
    canMsg.data[i] = 0;
  }  
}
...
void loop() {  
  ...
  packFloat(canMsg.data + 4, Speed);  // Add 4 to the pointer to shift to the next 4 bytes
